...
export default class CreateExercise extends Component {
  ...
  componentDidMount() {
    // this is wher the get is sent
    axios.get('http://localhost:5000/users/')
    // after request, take the respon and pass into arrow func
    // and check to see if there is a response
      .then(response => {
        if (response.data.length > 0) {
          // if there is at least 1 then do below
            // map method on data[] using map allows us return something for
            // each element in the array
            // for each user in the [] show each user
          this.setState({
